...
public:
    MarkerPositionNode() : Node("marker_position_node")
    {
        ...
        all_markers_pub_ = this->create_publisher<geometry_msgs::msg::PoseArray>("/all_marker_positions", 10);
...
private:
    ...
    void publish_marker_positions()
    {
        ...
        all_marker_positions.header.frame_id = "world";  // or "map" if that's your global frame
        ...
        for (const auto &robot_namespace : robot_names_)
        {
            for (const std::string &marker_name : {"marker_1", "marker_2", "marker_3", "marker_4", "marker_5", "marker_6", "marker_7"})
            {
                ...
                {
                    RCLCPP_WARN(this->get_logger(),
                                "Could not get transform for %s/%s: %s",
                                robot_namespace.c_str(),
                                marker_name.c_str(),
                                ex.what());
                }
