...
private:
  ...
  std::unordered_map<std::string, rclcpp::Publisher<geometry_msgs::msg::PolygonStamped>::SharedPtr> polygon_publishers_;
  ...
  void carOrientationsCallback(const geometry_msgs::msg::PoseArray::SharedPtr msg) {
    ...
    for (const auto &pose : msg->poses) {
        ...
        polygon_msg.header.frame_id = "robot0/odom";  // Adjust to your coordinate frame
        ...
        if (polygon_publishers_.find(topic_name) == polygon_publishers_.end()) {
            polygon_publishers_[topic_name] = this->create_publisher<geometry_msgs::msg::PolygonStamped>(topic_name, 10);
