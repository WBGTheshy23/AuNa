...
private:
  ...
  void poseCallback(const geometry_msgs::msg::PoseArray::SharedPtr msg) {
    ...
    pose_array_msg.header.frame_id = "robot0/odom";  // 依据你使用的坐标系调整
    ...
    for (const auto& [cluster_id, cluster_points] : clusters) {
      ...
      for (size_t i = 0; i < N; ++i) {
        for (size_t j = i + 1; j < N; ++j) {
          ...
          if (std::abs(rear_distance - 0.05) > 0.01) continue;  // 容差约 ±2cm
          ...
          // 尝试两个方向（前/后）寻找前中心 marker
          Eigen::Vector2d front_dir_candi[2] = {
            Eigen::Vector2d(-dir.y(), dir.x()),
            Eigen::Vector2d(dir.y(), -dir.x())
          };

          // 预期前中心marker位置
          Eigen::Vector2d expected_center_1 = rear_center + front_dir_candi[0] * 0.22;  // 0.22米前方
          Eigen::Vector2d expected_center_2 = rear_center + front_dir_candi[1] * 0.22;  // 0.22米前方
          ...
          if (center_idx != -1) {
            ...
            std::vector<int> front_markers;
            double length = 0.04; //juxing
            double width = 0.14;  //juxing
  
            for (size_t idx = 0; idx < N; ++idx) {
              if (used[idx]) continue;
              Eigen::Vector2d rel = rot * (cluster_points[idx] - rear_center);
              if (rel.x() > (0.34 - length / 2.0) && rel.x() < (0.34 + length / 2.0) && std::abs(rel.y()) < (width / 2.0 + 0.02)) {
            ...
            cars.push_back({front_dir,static_cast<int>(i), static_cast<int>(j), center_idx, front_markers});
  
            // 打印该车所有marker数（2后轮 + 1前中心 + 前轮区域markers）
            int total_markers = 2 + 1 + static_cast<int>(front_markers.size());
            RCLCPP_INFO(this->get_logger(),
                        "Cluster %d: Detected car with %d markers (rear left %d, rear right %d, front center %d, front region %ld)",
                        cluster_id, total_markers, (int)i, (int)j, center_idx, front_markers.size());
          }
        }
      }
      ...
      if (!all_marked) {
        RCLCPP_WARN(this->get_logger(), "Cluster %d has unassigned markers, possible mismatch.", cluster_id);
        break;
      } else {
        RCLCPP_INFO(this->get_logger(), "Cluster %d: Total detected cars: %ld", cluster_id, cars.size());
        ...
        for (const auto &car : cars) {
          ...
          id_marker.header.frame_id = "robot0/odom";
