  ...
  const rclcpp::Client<gazebo_msgs::srv::GetEntityState>::SharedFuture future)
{
  ...
  std::string highest_frame = "base_link";
  try {
    // Try to first find the transform from map to base_link
    highest_frame_to_base_link_lookup =
      buffer_.lookupTransform("map", "base_link", tf2::TimePointZero);
    highest_frame = "map";
  ...
  tf2::Quaternion q_ob(
    highest_frame_to_base_link_lookup.transform.rotation.x,
    highest_frame_to_base_link_lookup.transform.rotation.y,
    highest_frame_to_base_link_lookup.transform.rotation.z,
    highest_frame_to_base_link_lookup.transform.rotation.w);
